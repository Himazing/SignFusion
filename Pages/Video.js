...
function Video() {
  ...
  const extractVideoId = (url, platform) => {
    try {
      if (platform === "youtube") {
        const match = url.match(/(?:v=|youtu\.be\/|\/embed\/|watch\?v=|&v=)([A-Za-z0-9_-]{11})/);
        return match ? match[1] : null;
      } else if (platform === "instagram") {
        return url.includes("/reel/") || url.includes("/p/") ? url.split("?")[0] + "embed/" : null;
  ...
  const handleSubmit = (e) => {
    ...
    const videoId = extractVideoId(videoUrl, selectedPlatform.id);
    if (!videoId) {
      alert("Invalid URL. Please enter a valid video link for the selected platform.");
      return;
    }
    ...
    setEmbedUrl(embedUrls[selectedPlatform.id]);
  };
